...
workfolder = './cocoa/Cocoa/projects/roman_real/chains/roman_setup1_multievaluate/'
outname = 'chis.txt'
...
with open(os.path.join(rootdir, workfolder, outname), 'a', encoding='utf-8') as f:
    f.write("#As_1e9    ns    H0    omegab    omegam    omegach2    w0pwa   w    xi+    xi+_msk    xi-    xi-_msk    gammat    gammat_msk    wtheta    wtheta_msk    msk\n")
...
for i_cosmo in range(len(cosmologys)):
    ...
    dv_cosmolike = np.loadtxt(os.path.join(rootdir,f'./cocoa/Cocoa/projects/roman_real/chains/roman_setup1_multievaluate/roman.modelvector_{i_cosmo+1}'))[:,1]

    starts = [0, int(nsrcs*(nsrcs+1)/2*ntheta), int(nsrcs*(nsrcs+1)*ntheta), int((nsrcs*(nsrcs+1) + nlens*nsrcs)*ntheta), int((nsrcs*(nsrcs+1) + nlens*nsrcs + nlens)*ntheta)]
    output = [As_1e9[i_cosmo], ns[i_cosmo], H0[i_cosmo], omegab[i_cosmo], omegam[i_cosmo], omegach2[i_cosmo], w0pwa[i_cosmo], w[i_cosmo]]
    for i in range(4):
        l = starts[i]
        r = starts[i+1]
